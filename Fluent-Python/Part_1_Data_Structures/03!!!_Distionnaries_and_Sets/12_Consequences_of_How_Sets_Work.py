...
• Membership testing is very efficient. A set may have millions of elements, but an element can be located directly by
...
• Sets have a significant memory overhead, compared to a low-level array pointers to its elements, which would be more
  compact but also much slower to search beyond a handful of elements.
...
• Adding elements to a set may change the order of existing elements. That’s because the algorithm becomes less 
  efficient if the hash table is more than twothirds full, so Python may need to move and resize the table as it grows. 
